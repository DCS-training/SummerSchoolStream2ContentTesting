...
Below is the list of the packages that are needed for this class. Remember that if you are working on your machine you need to install the packages once and upload them every time that you close and open again R Studio. If you are working on Noteable you will have to re-install the packages at every new instance because they would not be retrained by the server

First the package installation

```{Install Packages,echo=FALSE}
...
```{r Mount the needed packages, results='hide'}

library(quanteda) #quanteda is an R package for managing and analyzing text
...
```
## Look around the Dataset
Let's refresh what we have in it
```{r summary, echo=TRUE}
summary(Parish)
```

This does not make much sense since we are looking at a fully textual dataset.
The next thing we need to do is to create a corpus out of the text column (that contains the text of each parish report)
```{r Corpus, echo=TRUE}
...
Remove the stop word. Stop words are a set of commonly used words in a language. Examples of stop words in English are “a,” “the,” “is,” “are,” etc. Stop words are commonly used in Text Mining and Natural Language Processing (NLP) to eliminate words that are so widely used that they carry very little useful information.
...
Let's search what is around words connected with whisky production and consuption
...
```
Something is off...there are definitely more "spirit" than what I would expect 
let's have a check
```{r Spirits, echo=TRUE}
...
For now, let's just remove it from the pool but this is a clear example on where blank computational text analysis may fail 
