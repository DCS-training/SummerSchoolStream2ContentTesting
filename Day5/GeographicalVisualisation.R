...
getwd()
setwd("/home/jovyan/_CDCS_summer")

Parish <- read.csv("data/Parishes.csv")
View(Parish)
...
## You can also change the basic format of the symbols
plot(ParishesGeo, col = "black", lwd = 0.5, border = "white", main = "Scottish Parishes")
plot(ParishesGeo, col = rgb(0.2, 0.6, 0.6), lwd = 0.5, border = "white", main = "Scottish Parishes")
...
# Create a continuous color palette
color.palette <- colorRampPalette(c("white", "red")) # colour changed from white to red to show polygons with NA values
...
## ssplot is quite inflexible with the legend, it might be easier if we use tm_shape to present the map, which is also compatible with sf. Therefore, students can still transfer the skills to using sf. although we focus on sp in the training.
tm_shape(MergedGeo) + tm_fill("per", style = "quantile", palette = color.palette(100), colorNA = "white") +
