...
readOGR Function: 
* from rgdal package that reads vector spatial data.
...
```
Besides the parish boundaries, we will also need the location of distilleries across Scotland. Load geospatial information for the location of distilleries. This is a vector point dataset.
```{r Gpkg4}
...
Try to re-plot the map using a different colour range using tmap
Add your code below 
```{r }
...
You can use classInt to make custom cuts if you don't want evenly spaced cutoffs between colors. For style, there are three basic methods ("equal", "sd" and "quantile"). "Equal" divides the range of values into n equal-sized intervals. "sd" represents how much values of a given variable varies from its mean, with each interval having a constant width of the standard deviation. "quantile" creates several classes with exactly the same number of objects (e.g., spatial features), but having intervals of various lengths. Number of intervals can be controlled with n.
...
Size can be adjusted using cex.
...
The last block of code checks if the difference between the maximum and minimum utilisation distribution values is less than a very small value (1e-9, which is close to 0). If the difference is indeed very small, it suggests that the utilisation distribution values are too small and will need to be scaled up for better visualisation. In this case, the code multiplies the entire kde.output object by 1e9 (1 billion) to scale up the values.
