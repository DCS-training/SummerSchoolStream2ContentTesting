...
author: "Rhys Davies & Lucia Michielin"
date: "`r Sys.Date()`"
...
After setting the working directory, we can upload our data! I tend to make life easier for myself at this stage - I use the graphic interface of R, then copy and paste the appropiate code into my code chunk.

Today, we're going to work on creating 2 separate tidy datasets; which we will then merge together at the end to give us a nice dataset to work with.
...
# Data Batch 1
scotch_reviews <- read_csv("data/scotch_review_manual_clean.csv")
List_of_whisky_distilleries_in_Scotland <- read_csv("data/List_of_whisky_distilleries_in_Scotland_manual_clean.csv")
...
Before cracking on with any data tidying, we first need to inspect, explore, scout, and have a good look around in our data. This will help us identify areas that need addressing, potential hiccups, and understand what needs to be done. We will aslo at this point be introduced to the pipe!
...
head(scotch_reviews, n = 6)  # head allows us to view the first `n` rows of a dataset
...
One of my favourite features of R is that we can apply these changes without changing our raw data! And better yet, we can keep track of the changes we've made - as the entire code is our notebook!! (I'm a PhD researcher... of course I'm a nerd who gets excited about this stuff).
...
\*Coding disclaimer: The `gsub()` function scares me. I haven't taken the time to fully understand what all the symbols are doing. But thankfully, the lovely people of stack overflow were able to help me find a [solution](https://stackoverflow.com/questions/78234787/extracting-conditional-numeric-values-from-character-data-in-r) for this problem.
...
2)  Plot the new `log_price` variable, and interpret it.
3)  Plot the new `ABV` varaible and interpret it.
...
Missing_ABV_scotch_review <- scotch_reviews_2 %>%
  ...
  filter(ABV_NA == "NA") # use this to filter so we only have the NA colums

Missing_ABV_scotch_review %>% gt() # Inspect the data to see what needs to be done
...
As the MCAR test is \> .05, the pattern of our missing data does not differ significantly from the missing completely at random structure (this is good). And we have a very low percentage of missing data. So, we are in a safe position to ignore our missing data and crack on with our data tidying.
...
#write_csv(df_merge, "Export/tidied_whisky_data.csv")
...
summary_data <- df_merge %>% 
  mutate(Old_v_new = case_when(Founded <= median(Founded) ~ "Old Distilleries", #creating an infamous median split...
                               Founded > median(Founded) ~ "Newer Distilleries")) %>%
...
summary_data %>% 
  ...
  tab_footnote( # addomg a footbote to explain the data
    footnote  = md("*'Old'* Distileries founded before 1839.<br> 'Newer' Distilleries founded after 1839.")
  )
```
...
wide_data <- summary_data %>% 
  ...
  select( # using select to reorder table - this way we can group by age
          Region, 
         `n_per_strata_Newer Distilleries`, 
         `mean_price_Newer Distilleries`,
...
prettier_table %>%
  cols_label(
    `n_per_strata_Newer Distilleries` = md("*n*"),
    `mean_price_Newer Distilleries` = md("*Mean*<br> Price ($)"),
    `n_per_strata_Old Distilleries` = md("*n*"), 
...
And there we have it. The end of our journey with data wrangling for today. We have covered:
